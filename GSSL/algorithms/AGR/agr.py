...
class AGR(KMeans):
    ...
    The Anchor Graph Regularization (ARG) algorithm described in [1].
    ...
    [1] W. Liu, J. He, and S. F. Chang, "Large graph construction for scalable
        semi-supervised learning", ICML, pp. 679-686 (2010).
    """
    ...
    def _compute_L(self):
        ...
        """
        W = self.W
        row_sums = self.Z.sum(axis=0).A1
        row_sums = 1 / row_sums
        self.L = W - W @ diags(row_sums) @ W
    ...
    def predict(
        ...
        if self.L is None:
            ...
            self.nnz["nnz_Z"] = self.Z.nnz  # Should be N*s
    ...
    def fit_predict(
        ...
        return self.predict(
            X=X, label_train=label_train, s=s, flag=flag, cn=cn, gamma=gamma
        )
