...
class MFAGLsparse(Gaussian):
    ...
    def _compute_Z(self, pjc, threshold):
        ...
        if threshold < 1.0:
            ...
            s = int(threshold)
            values, cols = to.topk(pjc, s, sorted=False)
            values /= values.sum(dim=1, keepdim=True)
            values, cols = values.flatten(), cols.flatten()
            rows = np.arange(pjc.shape[0]).repeat(s)
            return sparse.csr_matrix((values.numpy(), (rows, cols.numpy())))
    ...
    def _compute_labels(self, class_norm=True):
        ...
        if class_norm:
            Lambda = np.sum(prop_labels, axis=0)
            np.divide(1.0, Lambda, out=Lambda)
    ...
    def predict(
        ...
        """
        Performs label prediction (using sparse matrices).
        Used, if a threshold > 0 is set.
        """
